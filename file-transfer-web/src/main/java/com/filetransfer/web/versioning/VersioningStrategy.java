package com.filetransfer.web.versioning;

/**
 * Enumeration of supported API versioning strategies
 */
public enum VersioningStrategy {
    
    /**
     * Use the default configured versioning strategy
     */
    DEFAULT,
    
    /**
     * Version specified in URL path: /api/v1/tenants
     */
    URL_PATH,
    
    /**
     * Version specified in request header: X-API-Version: 1.0
     */
    HEADER,
    
    /**
     * Version specified in query parameter: ?version=1.0
     */
    QUERY_PARAMETER,
    
    /**
     * Version specified in Accept header: Accept: application/vnd.filetransfer.v1+json
     */
    MEDIA_TYPE,
    
    /**
     * Version specified in custom header: Accept-Version: 1.0
     */
    ACCEPT_HEADER,
    
    /**
     * Multiple strategies supported (fallback order)
     */
    HYBRID,
    
    /**
     * Semantic versioning with range support
     */
    SEMANTIC,
    
    /**
     * Date-based versioning
     */
    DATE_BASED,
    
    /**
     * No versioning (legacy endpoints)
     */
    NONE
}